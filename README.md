...
1. Выберите нужный вариант:
   - Все функции.
   - Все функции, кроме "ЭМОДЗИ".
   - Все функции, кроме "ТАЙМИНГ АВТОКЛИКА".
2. Скачайте готовый файл и переименуйте его в Assembly-CSharp.dll (если нужно).
...
5. Чтобы изменить код: нажмите правой кнопкой мыши на нужный класс и выберите "Edit Method (C#)...".
...
>По умолчанию в переменной ```UnityEngine.Random.Range(30, 300)``` задан случайный интервал от 30 до 300 = ~1309 эмодзи/ч (симуляция ввода). Частота отправки напрямую зависит от вашего FPS в игре, так как проверка условий происходит каждый кадр.
Формула расчета: Число из интервала / Ваш FPS = Задержка в секундах

>Пример ```UnityEngine.Random.Range(1, 250)``` (при 60 FPS):  
      Минимум (1): $1 / 60 = ~0.016$ сек.  
      Максимум (250): $250 / 60 = ~4.16$ сек.  
>Эмодзи будут отправляться рандомно каждые 0.016–4.16 секунд (~1724 эмодзи/ч). Если ваш FPS ниже 60, реальное время изменится. Вы можете изменить эти значения под себя, а также значение FPS в самой игре.
